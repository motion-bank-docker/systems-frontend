  ...
  full-screen
    ...
    q-modal(v-model="showModalAdd")
      .q-pa-xl.bg-dark(style="min-width: 50vw;")
        h5.caption.no-margin Create timeline
    ...
    q-modal(v-model="showModalDelete")
      .q-pa-lg.bg-red.text-white
        h6.no-margin.no-padding.text-center Do you really want to delete/leave this timeline?
        .text-center.q-mt-lg
    ...
    q-modal(v-model="showModalEditInformation")
      .q-pa-xl.bg-dark(style="min-width: 50vw;")
        ...
        .row.q-mt-md(v-model="watchTest")
          q-input.col-10(v-model="maps[0].title", float-label="Title", color="white", dark)
          .col-2.text-right
            q-btn.q-mt-md(:disable="disableBtn", label="Save", no-caps)

        // title
        .row.q-mt-md(v-model="watchTest")
          q-input.col-10(v-model="maps[0].desc", float-label="Description", color="white", dark)
          .col-2.text-right
            q-btn.q-mt-md(:disable="disableBtn", label="Save", no-caps)
    ...
    q-modal(v-model="showModalUsers")
      .q-pa-xl.bg-dark(style="min-width: 50vw;")
        ...
        q-list(no-border)
          q-item.q-my-sm.q-pa-md(style="border: 1px solid #333;")
            current-user
            | (Owner)
    ...
    span(slot="form-logo")
    div(slot="form-title")
      | {{ $t('routes.piecemaker.groups.list.title') }}
      = 'you have access to, '
      current-user
      ...
      .col-10.offset-1
        ...
        q-table.no-shadow(@rowclick="onRowClick($event)", :dark="true", :data="maps", :config="conf",
          ...
          q-td(slot="body-cell-users", slot-scope="props", :props="props")
            q-btn(@click="showModalUsers = true") {{ props.row.users.length }}

          q-td(slot="body-cell-actions", slot-scope="props", :props="props")
            q-btn(@click="onAction(a.type)", v-for="a in actions", :color="a.color || 'neutral'",
              :key="a.type", flat, no-caps, :icon="a.icon") {{ $t(a.title) }}
...
<script>
  ...
  import CurrentUser from '../../../components/shared/partials/CurrentUser'
  export default {
    components: {
      FullScreen,
      GroupList,
